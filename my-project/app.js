...
function renderResultScreen(userPick, computerPick, result) {
  ...
  // Show NEXT button only when user wins and game hasn't ended
  if (result === 'win' && turnCount < maxTurns) {
  ...
  // Auto-check for game end after all turns
  if (turnCount >= maxTurns) {
    setTimeout(() => {
      determineGameWinner();
    }, 2000);
...
function renderChoices() {
  gameArea.innerHTML = `
    ...
    <div id="rules-card" style="display: ${gameStarted ? 'none' : 'block'};">
  ...
  document.querySelectorAll('.choice').forEach(btn => {
    btn.onclick = () => {
      // Close rules card when game starts
      if (!gameStarted) {
        gameStarted = true;
        const rulesCardInGame = document.getElementById('rules-card');
        if (rulesCardInGame) {
          rulesCardInGame.style.display = 'none';
        }
      }
...
function renderHurrayScreen() {
  mainContainer.innerHTML = `
    <div class="hurray-wrapper">
      ...
      <div class="final-score">Final Score: You ${userScore} - ${computerScore} PC</div>
      <button class="picked-replay">PLAY AGAIN</button>
    </div>
...
function renderLostScreen() {
  mainContainer.innerHTML = `
     <div class="hurray-wrapper">
      ...
      <div class="hurray-title">GAME OVER</div>
      <div class="hurray-subtitle">PC WON THE GAME</div>
      <div class="final-score">Final Score: PC ${computerScore} - ${userScore} You</div>
      <button class="picked-replay">PLAY AGAIN</button>
    </div>
    <button class="rules-btn">RULES</button>
  `;
...
function renderTieScreen() {
  mainContainer.innerHTML = `
    <div class="hurray-wrapper">
      ...
      <div class="hurray-title">IT'S A TIE!</div>
      <div class="hurray-subtitle">BOTH PLAYED EQUALLY WELL</div>
      <div class="final-score">Final Score: ${userScore} - ${computerScore}</div>
      <button class="picked-replay">PLAY AGAIN</button>
    </div>
    <button class="rules-btn">RULES</button>
  `;
...
function showRulesModal() {
  const existingModal = document.getElementById('rules-modal-overlay');
  if (existingModal) {
  ...
  modal.innerHTML = `
    <div id="rules-card" style="display: block; position: fixed; top: 50%; right: 30px; transform: translateY(-50%); z-index: 1000;">
      <div class="rules-header">
        <h2>Game Rules</h2>
        <button type="button" class="close-btn">✕</button>
      </div>
...
// Rules button toggle
rulesBtn.onclick = () => {
  const currentRulesCard = document.getElementById('rules-card');
  if (currentRulesCard) {
    if (currentRulesCard.style.display === 'none' || !currentRulesCard.style.display) {
      currentRulesCard.style.display = 'block';
    } else {
      currentRulesCard.style.display = 'none';
    }
  } else {
    showRulesModal();
  }
